...
const BangCuuChuong1 = () => {
  ...
  const endGame = async () => {
    ...
    // Submit result to API (only for non-practice modes)
    if (selectedMode !== 'practice') {
      const playTime = Math.floor((Date.now() - startTime) / 1000);
      const accuracy = questions.length > 0 ? Math.round((correctAnswers / questions.length) * 100) : 0;
      await submitGameResult(score, playTime, accuracy);
    }
  };
          ...
          <div className="game-over-popup">
            ...
            <div className="stars-earned">
              ...
              <div className="stars-display">
                {Array.from({ length: starsEarned || stars }).map((_, idx) => (
                  <span key={idx} className="star-icon">⭐</span>
                ))}
              </div>
              <p className="stars-count">{starsEarned || stars} Sao!</p>
              {isSubmittingResult && <p className="submitting-text">Đang lưu kết quả...</p>}
              {starsEarned > 0 && <p className="success-text">✅ Đã lưu vào ví của bạn!</p>}
            </div>

            {selectedMode !== 'practice' && !starsEarned && (
